...
import {PlusIcon, CashIcon} from "@heroicons/vue/outline"
...
<template>
    <div class="flex flex-col justify-center items-center">
        ...
        <button @click="addAccount" v-if="!loading" class="flex items-center text-slate font-bold mt-2 fixed top-5 right-5 border-2 bg-white border-blue-200 p-2 gap-2 rounded shadow-xl hover:border-orange-300 hover:shadow-inner"><PlusIcon class="w-5 h-5"/>Add account</button>
        ...
        <table v-else class="table-auto m-auto mt-4 border-4 border-opacity-20 border-slate-500 w-full shadow">
            ...
            <tbody>
                <tr :class="index % 2 === 0 ? 'bg-slate-100' : 'bg-white'" class="border-b-2 border-slate-300 text-center" v-for="account, index in accounts" :key="account.accountId">
                    ...
                    <td>
                        <button v-tippy>Select
                            <tippy target="_parent" :interactive="true" placement="bottom">
